...
{
    private static int g_ObjectInstanceID = 0;
    private int m_hObjectInstanceID = 0;  protected virtual int GetObjectInstanceID() { return m_hObjectInstanceID; }
...
abstract public class CObjectInstancePoolBase<TEMPLATE> : CObjectBase where TEMPLATE : CObjectInstancePoolBase<TEMPLATE>
{
    private static List<TEMPLATE> g_listPoolInstance = new List<TEMPLATE>();
    private bool   m_bInstanceActive = false;

    public static INSTANCE InstancePoolEnable<INSTANCE>() where INSTANCE : TEMPLATE, new()
    {        
    ...
    public static void InstancePoolDisable(TEMPLATE pDeactiveInstance)
    {
    ...
    private static INSTANCE SearchObjectInstance<INSTANCE>() where INSTANCE : TEMPLATE, new()
    {
        ...
        {
            INSTANCE pInstaqnce = g_listPoolInstance[i] as INSTANCE;
            if (pInstaqnce.m_bInstanceActive == false)
            {
                pSearchInstance = g_listPoolInstance[i] as INSTANCE;
                break;
            }
        }
        ...
        if (pSearchInstance == null) 
        {
            ...
            g_listPoolInstance.Add(pSearchInstance);
        }

...
public class CPatcherAddressable : CPatcherBase
{
    ...
    protected override void OnPatcherUpdate(float DeltaTime)
    {
        if (mPatchStart)
        {
            if (mDownloadProgress.IsValid() && mDownloadProgress.IsDone == false && mDownloadProgress.Status != AsyncOperationStatus.Failed)
            {
                ...
                EventPatchProgress(ExtractAssetBundleLoadName(mDownloadProgress), currentDownload, mTotalDownloadSize, percent, loadCurrent, mTotalAssetBundleCount);
            }
        }
    }
    ...
    private void PrivPatchClearAssetBundle()
    {
        ...
        PrivPatcherExtractDownloadedAssetBundle(listAssetBundle);
      
        mTotalAssetBundleCount = (uint)listAssetBundle.Count;
        if (mTotalAssetBundleCount == 0) return;

        PrivPatcherCompareExpireAssetBundle(listAssetBundle);
    }

    private void PrivPatcherExtractDownloadedAssetBundle(List<AssetBundleRequestOptions> listAssetBundle)
    {
        // 다운로드된 에셋번들 목록 확보 

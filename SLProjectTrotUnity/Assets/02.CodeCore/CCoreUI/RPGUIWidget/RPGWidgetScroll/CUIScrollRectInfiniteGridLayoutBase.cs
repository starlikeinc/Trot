...
abstract public class CUIScrollRectInfiniteGridLayoutBase : CUIScrollRectBase
{
    ...
    /// </summary>
    public override sealed CUIWidgetTemplateItemBase GetUIScrollItemByViewportOffset(Vector2 vecViewportOffset)
    {
    ...
    //-----------------------------------------------------------------
    protected void ProtUIScrollInfiniteInitialize(int _totalCount, int iViewWidth, float fCellWidth = 0, float fCellHeight = 0, bool _scrollReset = true)
	{
		m_iTotalCount = _totalCount;
		...
		PrivUIScrollInfiniteSize(m_pGridLayout.cellSize, m_pGridLayout.spacing, ViewportSize, iViewWidth, _totalCount);
	...
	/// </summary>
	protected void ProtUIScrollInfiniteSetPosition(Vector2 _position) 
	{
		float positionX = Mathf.Clamp(_position.x, -m_pScrollRect.content.sizeDelta.x, 0);
		float positionY = Mathf.Clamp(_position.y, 0, m_pScrollRect.content.sizeDelta.y);
	...
	private void PrivUIScrollInfiniteSize(Vector2 _cellSize, Vector3 _spacing, Vector2Int _viewPortSize, int iConstrainCount, int _totalCount)
	{
        int SpacingCountX = iConstrainCount - 1;
		if (SpacingCountX < 0) SpacingCountX = 0;

		int ContentsWidth = ((int)_cellSize.x * iConstrainCount) + ((int)_spacing.x * (SpacingCountX)) - _viewPortSize.x;
		int ContentsHeightCount = _totalCount / iConstrainCount;
		if (_totalCount % iConstrainCount > 0)
		{
			ContentsHeightCount++;
		}
		...
		m_iGridCellWidthCount = iConstrainCount;

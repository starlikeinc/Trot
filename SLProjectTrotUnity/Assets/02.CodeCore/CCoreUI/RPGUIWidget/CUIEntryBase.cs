...
public abstract class CUIEntryBase : CMonoBase
{
	...
	internal void InterUIWidgetInitializePost(CUIFrameBase pParentFrame) // 모든 위젯들이 초기화 된 이후 호출. 버튼의 디폴트 프리셋등 다른 위젯과 상호작용 하는 기능 삽입
	{
		OnUIWidgetInitializePost(pParentFrame);
	}

	internal void InterUIWidgetOwner(CUIEntryBase pWidgetOwner)        // 위젯같의 상하관계가 있을 경우
	{
